...
(defun forward (&key from to thread)
  (declare (list from to))
  (destructuring-bind ((from-host from-port) (to-host to-port)) `(,from ,to)
                     ...
                     (with-stream (server-stream server)
                       (do-wait-for-input (in client-stream server-stream)
                         (if (eq in client-stream)
                             (stream-forward client-stream server-stream)
                           (stream-forward server-stream client-stream))))))))))
      (if thread
